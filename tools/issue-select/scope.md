...
I have worked in Python, JavaScript/TypeScript, and HTML/CSS, and I am
comfortable editing documentation. For a first contribution I prefer an
issue in one of those languages where the fix is a small, well-described
change to one or two files, ideally with a way to verify it (a test to
add, or a visible UI/doc change I can check by hand). I want to get
better at reading an unfamiliar codebase and writing a clean PR, so
issues that touch existing code and its tests rank above pure
copy-editing. Rank lower anything that needs a language or build system
I have not used, or that requires a large environment setup before the
first change can be made.
